...
| Final USB sync failed with Windows `ERROR 1006` and `ERROR 1392`; read-only `chkdsk F:` reports volume-bitmap and directory corruption | Final publication | Stop all writes to `F:`. Keep the verified NTFS clone and pushed GitHub branch as recovery sources. Run `chkdsk F: /F` only after explicit user approval, then re-sync and re-verify. |
